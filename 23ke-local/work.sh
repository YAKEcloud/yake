...
sleep 10 # TODO: wait for load balancer ip more reliably
...
helm upgrade --namespace=garden -i -f step-ca-values.yaml step-certificates smallstep/step-certificates
...
) += "--requeue-dependency=5s"' ../flux-system/gotk-components.yaml | kubectl apply -f -
...
echo ">>> waiting for hr gardener-application"
until kubectl get hr gardener-application -n flux-system; do
  sleep 3
done
echo ">>> ok"
