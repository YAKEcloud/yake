...
{{- required "Value domains.global.domain is required" (.Values.domains).global.domain }}
{{- end -}}
...
{{- else }}
{{- (print .Values.gardenlet.ingressDomain "." (include "global.domain" .)) }}
{{- end }}
{{- end -}}
...
{{- else }}
{{- (print "dashboard." (include "global.domain" .)) }}
{{- end }}
{{- end -}}
...
{{- else }}
{{- (print "identity." (include "global.domain" .)) }}
{{- end }}
{{- end -}}
...
{{- else }}
{{- (print "api." (include "global.domain" .)) }}
{{- end }}
{{- end -}}
...
{{- else }}
domain: internal.{{ (include "global.domain" .) }}
provider: {{ (.Values.domains).global.provider }}
credentials:
  {{- toYaml (.Values.domains).global.credentials | nindent 2 }}
{{- end }}
