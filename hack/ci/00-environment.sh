...
SHOOTMD5=$(md5 -q hack/ci/misc/shoot-template-hcloud-fsn1.yaml.tmpl)
DESIRED_PRESPAWNED_SHOOTS=2
set +e
ACTUAL_PRESPAWNED_SHOOTS=$(kubectl get shoots --selector=23technologies.cloud/free-to-use='true',23technologies.cloud/region=$ZONE,23technologies.cloud/shoot.yaml=$SHOOTMD5 --no-headers=true | cut -d ' ' -f4 | grep -c $PROVIDER)
set -e
...
do
    ...
    yq eval '.metadata.name = env(RANDNAME)' hack/ci/misc/shoot-template-$PROVIDER-$ZONE.yaml.tmpl |yq eval '.metadata.labels["23technologies.cloud/shoot.yaml"]=env(SHOOTMD5)' - | kubectl apply -f -
    set +e
...
SHOOT=$(kubectl get shoot --sort-by=.status.lastOperation.progress --no-headers=true --selector=23technologies.cloud/free-to-use='true',23technologies.cloud/region=$ZONE,23technologies.cloud/shoot.yaml=$SHOOTMD5 | grep "   $PROVIDER   " |cut -d ' ' -f1|tail -n1)
