...
kubectl patch -n garden-23ke-ci shoot $SHOOT --patch-file hack/ci/misc/scale-nodes-min4.patch > /tmp/stdout 2> /tmp/stderr || { echo -e "\rError while scaling shoot-cluster to 4 nodes. ❌" ; echo "STDOUT":; cat /tmp/stdout; echo "STDERR:"; cat /tmp/stderr; exit 1; }
echo -n "."
