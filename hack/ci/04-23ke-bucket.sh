...
echo -n "."
mc cp kustomization.yaml $MC_ALIAS/$BUCKET > /dev/null 2>&1 || { echo "Error while uploading 23KE root-ks Bucket ❌" ; exit 1; }
echo -n "."
mc cp 23ke-base-substitutions.yaml $MC_ALIAS/$BUCKET > /dev/null 2>&1 || { echo "Error while uploading 23KE-base-substitutions to Bucket ❌" ; exit 1; }
echo -n "."
mc cp --recursive flux/ $MC_ALIAS/$BUCKET > /dev/null 2>&1 || { echo "Error while uploading 23KE/flux to Bucket ❌" ; exit 1; }
echo -n "."
mc cp --recursive base-install $MC_ALIAS/$BUCKET > /dev/null 2>&1 || { echo "Error while uploading 23KE/base-install to Bucket ❌" ; exit 1; }
echo -n "."
mc cp --recursive base-config $MC_ALIAS/$BUCKET > /dev/null 2>&1 || { echo "Error while uploading 23KE/base-config to Bucket ❌" ; exit 1; }
echo -n "."
