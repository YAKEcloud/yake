...
yq eval '.data."values.yaml"' hack/dev-env/config/kube-apiserver-values.yaml.tmpl > /tmp/values.yaml
yq eval '.ingress.annotations."dns.gardener.cloud/dnsnames" = "api.dev.23ke-run-" + env(SHOOT) + ".23t-test.okeanos.dev' -i /tmp/values.yaml
export VALUES=$(cat /tmp/values.yaml)
yq eval '.data."values.yaml" = strenv(VALUES)' hack/dev-env/config/kube-apiserver-values.yaml.tmpl > hack/dev-env/config/kube-apiserver-values.yaml
