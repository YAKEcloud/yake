...
while [ ! "$(kubectl --context garden get shoot "$SHOOTNAME" -n "$SHOOTNAMESPACE" -o jsonpath="{.status.lastOperation.state}")" == "Succeeded" ]; do
  PERCENTAGE=$(kubectl --context garden get shoot "$SHOOTNAME" -n "$SHOOTNAMESPACE" -o jsonpath="{.status.lastOperation.progress}")
  echo "Creating shoot: $PERCENTAGE%"
	sleep 5
done
echo "Shoot creation succeeded"

kubectl get secret -n "$SHOOTNAMESPACE" "$SHOOTNAME".kubeconfig -o go-template='{{.data.kubeconfig|base64decode}}' --context garden > hack/ci/secrets/shoot-microservice-kubeconfig.yaml
...
kubectl wait --for=condition=available --timeout=600s -n default deployment/loadgenerator --kubeconfig hack/ci/secrets/shoot-microservice-kubeconfig.yaml \
  || { kubectl --context garden get shoot "$SHOOTNAME" -n "$SHOOTNAMESPACE" -o yaml; exit 1; }
