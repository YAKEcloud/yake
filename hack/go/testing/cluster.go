...
func UseKindCluster(ctx SpecContext, version string, withLoadBalancer bool) (string, string) {

	keepCluster := os.Getenv("KIND_CLUSTER_NAME") != ""
	...
	existingClusters, err := clusterProvider.List()
	Expect(err).NotTo(HaveOccurred())
	clusterExists := false
	for _, v := range existingClusters {
		if v == name {
			clusterExists = true
			break
		}
