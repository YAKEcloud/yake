#!/usr/bin/env bash
RANDOM=$(/usr/bin/env cat /dev/random | /usr/bin/env strings -e s -n 1 | /usr/bin/env tr -d -c 'cbdefghijklnrtuvCBDEFGHIJKLNRTUV0123456789' | /usr/bin/env dd bs=1 count=4 status=none)
